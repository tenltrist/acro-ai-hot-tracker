...
### 11.6 2026-07-29 按公司统一的十板块来源档案

平台总来源数不等于单家公司覆盖数。为避免用行业媒体、活动平台和共享新闻稿源的总数掩盖公司专属来源缺口，13 家公司统一使用以下 10 个来源槽位：
...
每个槽位显示 `公司专属`、`共享来源`、`专属 + 共享` 或 `尚未配置`，并使用 `专属运行`、`已有覆盖`、`待恢复 / 配置` 和 `待补入口` 表示建设状态。缺失槽位不从网页消失，而是作为待开发项明确保留。

数据源健康页选择公司后，会同时展示该公司的专属来源与已登记的共享覆盖源。共享源的候选数、日报数和最后日期必须按 `matched_company_ids` 二次切分，不得把整个公司池的产出计入某一家公司。完整配置位于 `config/company_source_coverage.json`。
