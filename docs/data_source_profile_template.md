...
| Bing News / 其他搜索聚合 | 备选补漏 | `paid_later` 或 `watchlist` |
...
| YouTube / Bilibili | Webinar、产品视频 | `watchlist` |
...
| 来源 | 价值 | MVP 处理 |
| --- | --- | --- |
| Google Scholar / PubMed | 论文、引用、技术影响 | `watchlist` |
| ClinicalTrials.gov / jRCT / EU CTIS | 临床项目关联 | `watchlist` |
| 专利数据库 | 技术布局、产品方向 | `watchlist` |
| FDA / EMA / PMDA | 监管与批准信号 | `watchlist` |
| 招聘网站 | 组织扩张、区域布局、技术方向 | `watchlist` |
| 公司财报 / SEC / EDINET | 战略、营收、风险披露 | `watchlist` / `paid_later` |
...
| `acro_publications` | PubMed / Google Scholar | `watchlist` | `manual_review` | B/D | 后续作为学术影响模块 |
