...
英文名称：ACRO Market Intelligence。副标题：公司动态 · 产品与技术 · 市场事件。
原名 ACRO AI Hot Tracker；仅调整对外名称，目录、仓库与分享网址沿用既有名称。

> 当前维护入口（2026-09-21）：[项目说明书](项目说明书/README.md)，包含功能地图、规则、分类、数据存储、AI/RAG边界、版本发布、设计与交接提示词。
> 本文件保留早期 MVP 介绍，部分“今天做什么、建议负责人、日报”等文案已滞后，不应单独作为当前运行规范；本地、远程与线上状态请看说明书的分项核对记录。
...
页面中的摘要会明确标注来源：自动流水线使用“规则提要”；高价值条目可导出到 ChatGPT Pro 批处理，核对原文后以“AI 情报精读”回填。当前自动运行不调用付费 LLM API，也不把规则模板冒充 AI 总结。
...
回填数据保存在 `data/manual_summaries.json`，以稳定 item id 与新闻关联；中文模式优先展示人工译题，摘要显示 `AI 情报精读`来源标识。未通过人工核对的译题或摘要不允许导入。
